...
VID_MODEL_PATHS=[
    "cnn_lstm_retinaface_finetuned.h5"
    "../cnn_lstm_retinaface_finetuned.h5"
    "../../cnn_lstm_retinaface_finetuned.h5"
]
...
VID_MODEL_PATH = None
for path in VID_MODEL_PATHS:
    if os.path.exists(path):
        VID_MODEL_PATH = path
        break

if VID_MODEL_PATH is None:
    raise FileNotFoundError(
        "Model file 'cnn_lstm_retinaface_finetuned.h5' not found! "
        "Please place it in the project root directory."
    )
...
vidmodel=keras.models.load_model(VID_MODEL_PATH)
# ====== HELPERS ======
...
async def predict_video(file: UploadFile = File(...)):
    ...
    if not (lower.endswith(".mp4") or lower.endswith(".avi") or lower.endswith(".mov")):
        raise HTTPException(
            status_code=400,
            detail="Please upload a valid video file (.mp4/.avi/.mov)."
        )
    ...
    try:
        ...
        # Save temporarily (OpenCV requires file path)
        temp_path = "temp_video.mp4"
        with open(temp_path, "wb") as f:
            f.write(contents)

        # ===== PREPROCESS (YOUR FUNCTION) =====
        sequence = preprocess_video(temp_path)

        if sequence is None:
            raise HTTPException(
                status_code=400,
                detail="Could not detect a face in the video."
            )

        # ===== MODEL PREDICTION =====
        preds = vidmodel.predict(sequence)[0]
        ...
        # ===== ALL PROBABILITIES =====
        all_probabilities = {
            CLASS_NAMES[i]: float(preds[i] * 100.0)
            for i in range(len(CLASS_NAMES))
        }
